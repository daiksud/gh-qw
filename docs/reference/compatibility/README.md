...
| `list` | Substring matching, smartcase, host-prefix matching, exact matching, shortest unique suffixes, and ascending output are supported. Linked worktrees add `@<slot>`. | Compatible matching with worktree extension | `internal/cmd/list.go` | `internal/cmd/list_test.go` |
