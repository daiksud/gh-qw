...
Use `vpr dev`, `vpr build`, `vpr preview`, `vpr check`, `vpr check:links`, `vpr check:smoke`,
`vpr test`, `vpr validate`, and `vpr format` for this site. Do not use `vp dev` or `vp build`: those are
...
`vpr check:smoke` serves the generated `dist` directory through Astro's preview server and checks
the rendered routes, first-party assets, Pagefind, and the not-found response over HTTP. It depends
on `vpr build` and is included in `vpr validate`.
